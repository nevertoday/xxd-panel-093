<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 093 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 093이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

복잡한 사진에서는 원근, 질감, 자잘한 디테일이 기억해야 할 핵심 형태와 관계를 가릴 수 있습니다. Panel 093은 정체성을 잃지 않으면서 그 시각적 소음을 정리합니다.

**Panel 093**은 주체, 자세, 구조, 서사 관계를 소수의 기하형과 자유형으로 직접 압축합니다. 사실적 원근과 매끈한 벡터 대신 겹침, 크기, 원본 사진에서 얻은 선명한 색, 미세하게 불규칙한 손그림 가장자리, 무광 과슈 질감으로 평면 공간을 만듭니다.

### 이런 경우에 적합합니다

- 복잡한 사진을 소수의 큰 형태로 즉시 읽히게 만들고 싶을 때
- 원본의 강한 색 기억을 선명하면서도 세련된 색 체계로 재구성하고 싶을 때
- 평면 공간, 손맛 있는 가장자리, 무광의 미드센추리 편집 감각을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 093로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 기하 평면색 · 평면 공간 · 선명한 원본 색 · 손그림 가장자리 · 무광 과슈
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 밝고 절제되며 촉감이 느껴지는 모더니즘 기하 평면색 일러스트 |
| **대표 특징** | 소수의 큰 기하형, 평면 공간, 선명한 색면, 손그림 가장자리, 무광 물감 질감 |
| **입력 존중 방식** | 정체성·자세·서사·색 기억을 유지하며 모든 원본을 도시 장면으로 강제하지 않음 |
...
```text
주체·윤곽·구조·자세·서사 관계 분석 → 복잡한 디테일과 반복 요소 통합 → 주체를 소수의 기하형과 자유형으로 직접 압축 → 겹침·크기·색면으로 평면 공간 구축 → 사진의 밝고 선명한 색 기억 재구성 → 손그림 가장자리와 무광 과슈/아크릴 질감 추가 → 극소량의 편집 타이포그래피를 기하 구성에 통합
```

## 완성작의 특징

- 주체 자체를 기하화하며 기하학 선으로 사실적 외곽만 따라 그리지 않습니다. 적은 수의 큰 형태로도 정체성을 알아볼 수 있어야 합니다.
- 사실적 부피를 약화하고 겹침, 크기, 색면의 적층과 높낮이 관계로 공간을 만듭니다.
- 사진에서 가장 밝고 기억에 남는 색으로 팔레트를 짜되 형광색이나 값싼 캔디색은 피합니다.
- 절제된 인문적 타이포그래피를 형태의 축, 가장자리, 여백에 통합합니다.
- 사실주의, 복잡한 원근, 무거운 그림자, 매끈한 벡터, 3D, 스티커풍, 템플릿 여행 포스터를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-093-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 093의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-093 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-093 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-093 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-093 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-093 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-093 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 093 적용, 공통 설정은 한 번만 확정
/xxd-panel-093 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-093`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-093-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-093-prompt.zh-CN.md)
...
| **[xxd-panel-093](https://github.com/nevertoday/xxd-panel-093)** | 기하 평면색 · 평면 공간 · 선명한 원본 색 · 손그림 가장자리 · 무광 과슈 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–093) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
