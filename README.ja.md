<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 093 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 093 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真に含まれる遠近、質感、細かな要素が多すぎると、本当に記憶に残る形や関係が見えにくくなります。Panel 093 は、同一性を失わずにその視覚的なノイズを整理します。

**Panel 093** は主体、姿勢、構造、物語関係を少数の幾何形と自由形へ直接圧縮します。写実的な遠近や滑らかなベクターの代わりに、重なり、尺度、写真由来の鮮やかな色、わずかに不規則な手描きの縁、マットなガッシュの質感で平面空間を構成します。

### 向いている用途

- 複雑な写真を、少数の大きな形によってひと目で読める画面にしたい場合
- 元写真の強い色の記憶を、鮮やかでありながら洗練された配色へ再編したい場合
- 平面空間、手仕事を感じる縁、マットなミッドセンチュリー編集表現を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 093 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 幾何平塗り · 平面空間 · 鮮明な写真由来色 · 手描きの縁 · マットなガッシュ
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 明快で抑制的、素朴かつ上質なモダニズム幾何平塗りイラスト |
| **識別しやすい特徴** | 少数の大きな幾何形、平面空間、鮮明な色面、手描きの縁、マットな絵具肌 |
| **入力の尊重** | 同一性、姿勢、物語、色の記憶を保ち、どの写真も無理に都市題材へ変えない |
...
```text
主体・輪郭・構造・姿勢・物語関係を分析 → 複雑な細部と反復要素を統合 → 主体を少数の幾何形と自由形へ直接圧縮 → 重なり・尺度・色面で平面空間を構築 → 写真の明るく鮮やかな色の記憶を再編 → 手描きの縁とマットなガッシュ／アクリル質感を加える → 極少量の編集文字を幾何構成へ統合
```

## 完成作の特徴

- 主体そのものを幾何化し、幾何線で写実輪郭をなぞらない。少数の大きな形でも同一性を認識できるようにする。
- 写実的な体積を弱め、重なり、尺度、色面の積層、高低関係で空間を作る。
- 写真の最も明るく記憶に残る色から配色を組み、鮮やかさを保ちながら蛍光色や安価なキャンディ色を避ける。
- 控えめで人文的な文字を、形の軸、縁、余白へ統合する。
- 写実、複雑な遠近、重い影、滑らかなベクター、3D、ステッカー風、テンプレート旅行ポスターを避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-093-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 093 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-093 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-093 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-093 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-093 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 093 を適用し、共通設定は一度だけ解決
/xxd-panel-093 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-093`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-093-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-093-prompt.zh-CN.md)
...
| **[xxd-panel-093](https://github.com/nevertoday/xxd-panel-093)** | 幾何平塗り · 平面空間 · 鮮明な写真由来色 · 手描きの縁 · マットなガッシュ |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–093） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
