<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 093 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 093 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、结构、姿态与叙事关系 → 合并复杂细节与重复元素 → 将主体直接压缩成少量几何和自由块面 → 用遮挡、尺度与色块叠压建立扁平空间 → 从照片提取明亮鲜活的色彩记忆并重新组织 → 加入轻微手绘边缘与哑光水粉／丙烯肌理 → 让极少量文字成为几何构图的一部分
```

## 成品中最容易识别的风格特点

- 主体本身被几何化，而不是用几何线条描摹写实轮廓；少量大形状仍保留一眼可辨的身份。
- 透视和体积被主动弱化，空间主要由前后遮挡、尺度变化、色块叠压与高低关系建立。
- 色彩从照片最明亮、最鲜活、最具记忆点的部分出发，适度提亮、提纯、去灰，但不落入荧光或廉价糖果感。
- 颜料表面细腻哑光，边缘带极轻微刷痕、偏差和剪纸感；文字沿色块和主体轴线成为构图的一部分。
- 拒绝写实插画、复杂透视、厚重光影、光滑矢量、3D 渲染、卡通贴纸和模板化旅游海报。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-093-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–093） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
